...
def upload(cfg: UploadConfig) -> None:
    print(f"[*] Uploading from `{cfg.run_dir}` to HF Hub :: `{cfg.hub_repo}/{cfg.model_id}`")
    ...
    model_state_dict = torch.load(cfg.run_dir / "checkpoints" / "latest-checkpoint.pt", map_location="cpu")
    for mkey in model_state_dict["model"]:
        for pkey in model_state_dict["model"][mkey].keys():
            assert model_state_dict["model"][mkey][pkey].dtype == torch.float32, "Expected Tensor in FP32!"
            model_state_dict["model"][mkey][pkey] = model_state_dict["model"][mkey][pkey].to(torch.bfloat16)

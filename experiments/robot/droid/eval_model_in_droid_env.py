"""
eval_model_in_droid_env.py

Runs a model checkpoint in a real-world DROID Panda robot environment.

Usage:
    OpenVLA:
        Note: Set `center_crop==False` if not using random crop image aug'

        python experiments/robot/tdroid/eval_model_in_droid_env.py \
            --model_family llava \
            --model.type <VLM_TYPE> \
...
class GenerateConfig:
    ...
    gripper_action_space: str = "position"                     # Change this to "velocity" for DP
    unnorm_key: str = ""
...
def main(cfg: GenerateConfig) -> None:
    ...
    if cfg.model_family == "llava":
        # Use OpenVLA model server for DROID eval -- no local model needed
        model = None
    else:
